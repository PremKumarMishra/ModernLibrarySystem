...
public class MemberEditModalController
{
    @FXML
    ...
    public void initValues(String userName,String email,String regd,int ID)
    {
        this.userNameField.setText(userName);
        this.emailField.setText(email);
        this.regdField.setText(regd);
    ...
    protected void updateMember()
    {
        if (!userNameField.getText().isBlank() && !emailField.getText().isBlank() && !regdField.getText().isBlank())
        {
            ...
            {
                String sqlQuery = "UPDATE MEMBERS SET NAME = \"%s\", EMAIL=\"%s\", REGD=\"%s\" WHERE ID = %d;".formatted(userNameField.getText(),emailField.getText(),regdField.getText(),memberID);
